...
if __name__ == "__main__":
    ...
    # Path to the folder containing .tga files (always <root>/gfx/flags)
    folder_path = os.path.join(root_dir, "gfx", "flags")

    # Path to the countries.txt file (always <root>/common/countries.txt)
    countries_file = os.path.join(root_dir, "common", "countries.txt")

    # Verify that the expected paths exist before proceeding
    if not os.path.isdir(folder_path):
        print(f"Error: Flags folder not found at '{folder_path}'")
        exit(1)

    if not os.path.isfile(countries_file):
        print(f"Error: countries.txt not found at '{countries_file}'")
        exit(1)
